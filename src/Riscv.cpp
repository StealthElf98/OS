...
//            TCB::dispatch();
            ...
            TCB::dispatch();
        } else if(opCode == T_SEND) {
            TCB* tHandle;
            char* message;

            __asm__ volatile ("ld %0, 88(fp)" : "=r"(tHandle));
            __asm__ volatile ("ld %0, 96(fp)" : "=r"(message));

            TCB::running->send(tHandle, message);
        } else if(opCode == T_REC) {
            char* val = TCB::running->receive();
